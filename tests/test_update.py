    ...
    with psycopg.connect(clean_db_for_update) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                FROM values_table 
                WHERE run_id = %s AND tenant_id = %s AND valid_time = %s AND series_id = %s AND is_current = true
                """,
                (sample_run_id, sample_tenant_id, sample_datetime, sample_series_id)
            )
            row = cur.fetchone()
...
def test_update_record_annotation_only(clean_db_for_update, sample_run_id, sample_tenant_id, sample_series_id, sample_workflow_id, sample_datetime):
